...
class Config:
    ...
    FUEL_PRICE_PER_LITER = 180.0  # Kenyan Shillings per liter (changed to float)
    AVERAGE_FUEL_CONSUMPTION = 0.12  # Liters per kilometer for ambulances
    BASE_OPERATING_COST_PER_KM = 50.0  # KSh per km (changed to float)
    FUEL_TANK_CAPACITY = 80.0  # Liters - typical ambulance fuel tank (changed to float)
...
class CostManagementUI:
    ...
    def display_fuel_management(self):
        ...
        col1, col2 = st.columns(2)
        with col1:
            # FIXED: Ensure all values are the same type (float)
            current_price = st.number_input("Current Fuel Price (KSh/L)", 
                                          value=float(Config.FUEL_PRICE_PER_LITER),  # Convert to float
                                          min_value=100.0, max_value=300.0, step=1.0)
        with col2:
            if st.button("Update Fuel Price", use_container_width=True):
                Config.FUEL_PRICE_PER_LITER = float(current_price)  # Ensure it's stored as float
    ...
    def display_budget_planning(self):
        ...
        avg_trip_cost = 1500  # Average cost per trip in KSh
        projected_cost = expected_trips * avg_trip_cost
        projected_savings = projected_cost * 0.15  # 15% savings
...
class DriverUI:
            ...
            if patient:
                ...
                with col2:
                    ...
                    with st.form("location_update_form"):
                        # FIXED: Ensure all number inputs have consistent types
                        new_lat = st.number_input("Latitude", value=float(ambulance.latitude or -0.0916), format="%.6f")
                        new_lng = st.number_input("Longitude", value=float(ambulance.longitude or 34.7680), format="%.6f")
    ...
    def update_location_form(self, ambulance):
        with st.form("location_form"):
            ...
            # FIXED: Ensure consistent float types
            latitude = st.number_input("Latitude", value=float(ambulance.latitude or -0.0916), format="%.6f")
            longitude = st.number_input("Longitude", value=float(ambulance.longitude or 34.7680), format="%.6f")
            if st.form_submit_button("Update Location", use_container_width=True):
                ambulance_service = AmbulanceService(self.db)
                if ambulance_service.update_ambulance_location(
